...
class Handler {
  ...
  async getNatives () {
    ...
    if (this.getMinecraftMinor(this.version.id) >= 19) return this.options.overrides.cwd || this.options.root
    ...
    if (!fs.existsSync(nativeDirectory) || !fs.readdirSync(nativeDirectory).length) {
      ...
      await Promise.all(stat.map(async (native) => {
        if (!native) return
        const name = (native.path || native.url || '').split('/').pop()
  ...
  isModernForge (json) {
    return json.inheritsFrom && this.getMinecraftMinor(json.inheritsFrom) >= 12 && !(json.inheritsFrom === '1.12.2' && (json.id.split('.')[json.id.split('.').length - 1]) === '2847')
  }
