  ...
  async launch (options) {
    try {
      ...
      if (this.handler.getOS() === 'osx') {
        if (this.getMinecraftMinor(versionFile.id) > 12) jvm.push(await this.handler.getJVM())
      ...
      const mcMinor = this.getMinecraftMinor(versionFile.id)
      if (mcMinor === 18 && !parseInt(versionFile.id.split('.')[2])) jvm.push('-Dlog4j2.formatMsgNoLookups=true')
      if (mcMinor === 17) jvm.push('-Dlog4j2.formatMsgNoLookups=true')
      if (mcMinor < 17) {
      ...
      this.emit('debug', `[MCLC]: Failed to start due to ${e.stack || e}, closing...`)
      return null
    }
  }

  getMinecraftMinor (versionId) {
    if (!versionId) return 0
    const parts = String(versionId).replace(/[^0-9.]/g, '').split('.').map(Number)
    const major = parts[0] ?? 0
    if (major !== 1 && major >= 2) return 999
    return parts[1] ?? 0
  }
