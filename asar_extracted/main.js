...
// ── Enriquecer version JSON con --userProperties para skins de cuentas Nebula ──
function enrichVersionJsonWithUserProperties(jsonPath) {
    try {
        if (!fs.existsSync(jsonPath)) return;
        const vJson = JSON.parse(fs.readFileSync(jsonPath, 'utf8'));
        const gameArgs = vJson.arguments?.game;
        if (!gameArgs) return;
        // Solo modificar si no tiene ya --userProperties
        if (gameArgs.includes('--userProperties')) return;
        // Insertar --userProperties despues de --versionType (ultimo arg fijo antes de los rule-based)
        let insertIdx = -1;
        for (let i = 0; i < gameArgs.length; i++) {
            if (gameArgs[i] === '--versionType') { insertIdx = i + 2; break; }
        }
        if (insertIdx === -1) insertIdx = gameArgs.length;
        gameArgs.splice(insertIdx, 0, '--userProperties', '${user_properties}');
        fs.writeFileSync(jsonPath, JSON.stringify(vJson, null, 2));
    } catch (e) {
        // Silencioso: si falla, el juego igual se lanza (sin skin)
    }
}
...
ipcMain.on('launch-game', async (event, data) => {
    ...
    try {
        ...
        // Inyectar --userProperties en el JSON de version para skins Nebula
        try {
            const launchVerId = versionOpts.custom || launchVersion;
